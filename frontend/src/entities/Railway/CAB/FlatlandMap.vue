    ...
    <div class="flatland-controls">
      ...
      <div class="ctrl-speed">
        ...
        <input
          ...
          :title="sessionLocked ? 'Tempo kann während eines Szenarios nicht geändert werden' : ''"
          min="0.5"
          max="5"
          step="0.5"
...
let speedLockTimer: any
onMounted(() => {
  speedLockTimer = setInterval(async () => {
    try {
...
onUnmounted(() => clearInterval(speedLockTimer))
...
.ctrl-btn-session:hover { background: #3a8a52; }

.ctrl-btn-session-end {
  background: #6a2d2d;
  font-size: 12px;
}
.ctrl-btn-session-end:hover { background: #8a3a3a; }
